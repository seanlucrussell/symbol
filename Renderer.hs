...
renderTerm'' :: Renderable a => Path -> SymbolTable -> RenderContext a -> Term a -> Doc Marking
renderTerm'' path s c (Term t ts) = annotate (Location path) (renderTerm' s c t renderedChildren)

...
--     Function [] b               -> Just $ ZipperAs  b (FnBody [] ts) 
...
--     FnArgs a [] s ts                 -> Just $ ZipperAs  s (FnBody (d:a) ts)
...
--     DeclareType n (FnArgs a [] s ts) -> Just $ ZipperAs  s (FnBody (Declare n t:a) ts)
...
-- insertBefore (ZipperAs s (TopLevel a b)) = ZipperAs s (TopLevel (UnknownStatement:a) b) .- selectPrev
-- insertBefore (ZipperAs s (FnBody a xs)) = ZipperAs s (FnBody (a ++ [Declare UnknownName UnknownType]) xs) .- selectPrev .- selectPrev
...
-- insertAfter (ZipperAs s (TopLevel a b)) = ZipperAs s (TopLevel a (UnknownStatement:b)) .- selectNext
...
--                  , Function UnknownValue UnknownValue
